...
// Function to get user's favorite drink
...
// Route to get user's favorite drink
...
// Route to add user's favorite drink
router.post('/', async (req, res) => {
  try {
    const userId = req.session.user_id;

    // Assuming req.body has the necessary information for the favorite drink
    ...
    console.error('Error adding user\'s favorite drink:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Route to update user's favorite drink
router.put('/:id', async (req, res) => {
  try {
    const userId = req.session.user_id;
    const favoriteId = req.params.id;

    // Assuming req.body has the updated information for the favorite drink
...
// Route to delete user's favorite drink
